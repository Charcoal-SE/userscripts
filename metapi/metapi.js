...
var mp = function ($) {
  ...
  var fieldMapRequest = $.ajax({
    type: 'GET',
    url: 'https://metasmoke.erwaysoftware.com/api/filter_fields'
  });
  fieldMapRequest.done(function (data) {
    api_field_mappings = data;
  });
  fieldMapRequest.fail(function (jqXhr) {
  ...
  return {
    ...
    Cache: function () {
       ...
       */
      this.get = function (k, options) {
        options = options || {};
        if (options.hasOwnProperty("throwOnFail") && options['throwOnFail']) {
          if (store.hasOwnProperty(k)) {
            return store[k];
          }
          throw new ReferenceError("Specified key does not exist in cache.");
        }
        else {
          return store[k] || null;
        }
     ...
     */
    Filter: function (required_fields) {
     ...
     */
    WebSocket: function (address, onOpen) {
      var callbacks = [];
     ...
     */
    getPost: function (ident, key, options, callback) {
      options = options || {};
      options.key = key;
     ...
     */
    swapCodeForToken: function (code, key, callback) {
      $.ajax({
        type: "GET",
     ...
     * @param callback  a callback function accepting a metapi.Response as a single parameter
     */
    sendFeedback: function (id, feedback, key, token, callback) {
      $.ajax({
        type: "POST",
     ...
     * @param callback  a callback function accepting a metapi.Response as a single parameter
     */
    reportPost: function (url, key, token, callback) {
      $.ajax({
        type: "POST",
     ...
     * @param callback  a callback function accepting a metapi.Response as a single parameter
     */
    spamFlagPost: function (id, key, token, callback) {
      $.ajax({
        type: "POST",
     ...
     *                         websocket
     */
    watchSocket: function (key, messageCallback) {
      var sock;
...
if (typeof window === "undefined") {
  // We're probably running in Node for tests.
  exports.metapi = mp;
}
else {
  // We're probably running in the browser.
  window.metapi = mp(window.$);
}
