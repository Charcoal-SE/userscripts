...
(function() {
  ...
  const userscript = function($) {
    window.fdsc = {};
    fdsc.metasmokeKey =
      "070f26ebb71c5e6cfca7893fe1139460cf23f30d686566f5707a4acfd50c";
    fdsc.ready = false;
    ...
    */
    fdsc.input = function(blurb, callback) {
      ...
      $("body").loadPopup({
        ...
        target: $("body"),
        html:
          "<div class='popup fdsc-popup' id='fdsc-popup-prompt'><p>" +
          blurb +
          "</p><input type='text' id='fdsc-popup-input' /><br/><button id='fdsc-popup-submit'>OK</button></div>",
        loaded,
      });
    };
    ...
    fdsc.confirm = function(blurb, callback) {
      ...
      $("body").loadPopup({
        ...
        target: $("body"),
        html:
          "<div class='popup fdsc-popup' id='fdsc-popup-confirm'><p>" +
          blurb +
          "</p><button style='margin:5px;' id='fdsc-popup-ok'>OK</button><button style='margin:5px;' id='fdsc-popup-cnl'>Cancel</button></div>",
        loaded,
      });
    };
    ...
    */
    fdsc.getWriteToken = function(afterFlag, callback) {
      ...
      window.open(
        "https://metasmoke.erwaysoftware.com/oauth/request?key=" +
          fdsc.metasmokeKey,
        "_blank"
      );
      ...
      function getInput() {
        fdsc.input(
          "Once you've authenticated FDSC with metasmoke, you'll be given a code; enter it here.",
          code => {
            debug("input callback: " + code);
            $.ajax({
              url:
                "https://metasmoke.erwaysoftware.com/oauth/token?key=" +
                fdsc.metasmokeKey +
                "&code=" +
                code,
              method: "GET",
              ...
              .error(jqXHR => {
                if (jqXHR.status === 404) {
                  StackExchange.helpers.showErrorMessage(
                    $(".topbar"),
                    "metasmoke could not find a write token - did you authorize the app?",
                    {
                      position: "toast",
                      transient: true,
                  ...
                  StackExchange.helpers.showErrorMessage(
                    $(".topbar"),
                    "An unknown error occurred during OAuth with metasmoke.",
                    {
                      position: "toast",
                      transient: true,
                      transientTimeout: 10000,
    ...
    */
    fdsc.sendFeedback = function(feedbackType, postId) {
      ...
      $.ajax({
        type: "POST",
        url:
          "https://metasmoke.erwaysoftware.com/api/v2.0/feedbacks/post/" +
          postId +
          "/create",
        data: {
          type: feedbackType,
        ...
        .done(data => {
          StackExchange.helpers.showSuccessMessage(
            $(".topbar"),
            "Fed back " + feedbackType + " to metasmoke.",
            {
              position: "toast",
              transient: true,
              transientTimeout: 10000,
        ...
        .error(jqXHR => {
          if (jqXHR.status === 401) {
            StackExchange.helpers.showErrorMessage(
              $(".topbar"),
              "Can't send feedback to metasmoke - not authenticated.",
              {
                position: "toast",
                transient: true,
                transientTimeout: 10000,
            ...
            console.error(
              "fdsc.sendFeedback was called without having a valid write token"
            );
            fdsc.confirm(
              "Write token invalid. Attempt re-authentication?",
              result => {
                if (result) {
            ...
            StackExchange.helpers.showErrorMessage(
              $(".topbar"),
              "An error occurred sending post feedback to metasmoke.",
              {
                position: "toast",
                transient: true,
                transientTimeout: 10000,
      ...
      $.ajax({
        ...
        .error(jqXHR => {
          if (jqXHR.status === 401) {
            StackExchange.helpers.showErrorMessage(
              $(".topbar"),
              "Can't report post to metasmoke - not authenticated.",
              {
                position: "toast",
                transient: true,
                transientTimeout: 10000,
            ...
            console.error(
              "fdsc.reportPost was called without having a valid write token"
            );
            fdsc.confirm(
              "Write token invalid. Attempt re-authentication?",
              result => {
            ...
            StackExchange.helpers.showErrorMessage(
              $(".topbar"),
              "An error occurred while reporting the post to metasmoke.",
              {
                position: "toast",
                transient: true,
                transientTimeout: 10000,
      ...
      initCallback() {
        ...
        $(".flag-post-link").on("click", clickEvent => {
          $(document).on("DOMNodeInserted", nodeEvent => {
            let container;
            if (
              $(nodeEvent.target).hasClass("popup") &&
              $(nodeEvent.target).attr("id") === "popup-flag-post"
            ) {
              container = $(clickEvent.target)
              ...
              fdsc.ajaxPromise = $.ajax({
                ...
                .done(data_ => {
                  ...
                  function registerFeedbackButton(
                    buttonSelector,
                    feedback,
                    logMessage
                  ) {
                    $(buttonSelector).on("click", ev => {
                      ...
                      if (!fdsc.msWriteToken || fdsc.msWriteToken === "null") {
                        fdsc.getWriteToken(true, () => {
                          fdsc.sendFeedback(
                            feedback,
                            $(nodeEvent.target).attr("data-fdsc-ms-id")
                          );
                        });
                      } else {
                        fdsc.sendFeedback(
                  ...
                  if (data_.length > 0 && data_[0].id) {
                    ...
                    $.get(
                      ...
                      data => {
                        ...
                        const fpButtonStyle =
                          "style='color:rgba(255,0,0,0.5);' onMouseOver='this.style.color=\"rgba(255,0,0,1)\"' onMouseOut='this.style.color=\"rgba(255,0,0,0.5)\"'";
                        const tpButtonStyle =
                          "style='color:rgba(0,100,0,0.5);' onMouseOver='this.style.color=\"rgba(0,100,0,1)\"' onMouseOut='this.style.color=\"rgba(0,100,0,0.5)\"'";
                        let status =
                          "<div style='float:left' id='smokey-report'><strong>Smokey report: <span style='color:darkgreen'>" +
                          tps +
                          " tp</span>, <span style='color:red'>" +
                          fps +
                          " fp</span>, <span style='color:#7c5500'>" +
                          naa +
                          " naa</span>, " +
                          fdsc.autoflagged +
                          "</strong>";
                        ...
                        if (
                          !fdsc.msWriteToken ||
                          fdsc.msWriteToken === "null"
                        ) {
                          status +=
                            " - <a href='#' id='get-write-token'>get write token</a></div>";
                          writeTokenButton = true;
                        }

                        if (isFlagged || isAutoflagged) {
                          status +=
                            " - <a href='#' id='autoflag-tp' " +
                            tpButtonStyle +
                            ">tpu-</a></div>";
                        }
                        ...
                        if (tps === 0) {
                          status +=
                            " - <a href='#' id='feedback-fp' " +
                            fpButtonStyle +
                            ">false positive?</a></div>";
                        } else {
                        ...
                        registerFeedbackButton(
                          "#feedback-fp",
                          "fp-",
                          "Reporting as false positive"
                        );
                        // On click of the confirm autoflag
                        registerFeedbackButton(
                          "#autoflag-tp",
                          "tpu-",
                          "Reporting as true positive"
                        );
                ...
                .error(jqXHR => {
                  StackExchange.helpers.showMessage(
                    $(".topbar"),
                    "An error occurred fetching post from metasmoke - has the post been reported by Smokey?",
                    {
                      position: "toast",
                      transient: true,
                      transientTimeout: 10000,
            ...
            $(".popup-submit").on("click", () => {
              ...
              let feedbackType;
              if (
                selected.val() === "PostSpam" ||
                selected.val() === "PostOffensive"
              ) {
                feedbackType = "tpu-";
  ...
  */
  ...
  sourceEl.src =
    "https://cdn.rawgit.com/ofirdagan/cross-domain-local-storage/d779a81a6383475a1bf88595a98b10a8bd5bb4ae/dist/scripts/xdLocalStorage.min.js";
